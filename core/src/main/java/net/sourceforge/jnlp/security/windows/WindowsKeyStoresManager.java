...
public class WindowsKeyStoresManager {
	...
	static {
		...
		boolean windowsMachine = OS_NAME.indexOf("Windows") >= 0;
		if(windowsMachine) {
			//Load configurations. Only for Windows OS machines.
			WindowsKeyStoresManager winStore = new WindowsKeyStoresManager();
	        ...
	        winStore.exist = windowsRoot || windowsMy;
	        
	        if(windowsRoot && windowsMy) {
	        	winStore.type = WindowsKeyStoreType.WINDOWS_ROOT;
	        }
	        else if(!windowsRoot && windowsMy) {
	        	winStore.type = WindowsKeyStoreType.WINDOWS_MY;
	        }
	        else if(windowsRoot && !windowsMy) {
	        	winStore.type = WindowsKeyStoreType.WINDOWS_ROOT;
	        }
